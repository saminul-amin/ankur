...
export function ResultSummary({ mcqGrade, writtenEvaluation, activitySet, source, performance, revisionPending = false, onRevise, onNewSource }: Readonly<{
  mcqGrade: McqGrade;
  ...
  return <LazyMotion features={domAnimation}><div className="result-stage">
    ...
    <section className="weak-concepts" aria-labelledby="weak-title"><header><span><Sprout aria-hidden="true" /></span><div><p className="eyebrow">Next growth edge</p><h3 id="weak-title">{weak.length === 0 ? "All assessed concepts are mastered" : "Review targets, ordered by urgency"}</h3></div></header>{weak.length === 0 ? <p>No weakness is being fabricated. You can stop here or choose one optional challenge retry for the lowest-scoring mastered concept.</p> : <ol>{weak.map((item) => <li key={item.conceptId}><AlertTriangle aria-hidden="true" size={18} /><div><strong lang={source.language === "bn" ? "bn" : undefined}>{item.name}</strong><span>{item.strength.replaceAll("_", " ")} · {item.percentage}% · {item.priority} priority</span></div></li>)}</ol>}<div className="revision-coming revision-coming--ready"><Flower2 aria-hidden="true" /><div><strong>{weak.length === 0 ? "Optional challenge" : "Personalized revision is ready"}</strong><p>Generate source-grounded notes and a differently worded two-question retry. Your completed result remains saved.</p></div><Badge tone="sprout">Adaptive</Badge></div></section>
    <div className="stage-actions"><Button variant="quiet" onClick={onNewSource}><Sparkles aria-hidden="true" size={16} />Start a new source</Button><Button disabled={revisionPending} onClick={onRevise}><Flower2 aria-hidden="true" size={16} />{revisionPending ? "Growing revision…" : weak.length === 0 ? "Take challenge retry" : "Build revision plan"}</Button></div>
  </div></LazyMotion>;
}
