...
async function main(): Promise<void> {
  if (process.env["ANKUR_TASK06C_R2E_PREFLIGHT_OPT_IN"] !== "true") {
    throw new Error("TASK06C_R2E_PREFLIGHT_OPT_IN_REQUIRED");
  }
  ...
  for (let ordinal = 1; ordinal <= REQUEST_COUNT; ordinal += 1) {
    ...
    try {
      const result = await provider.generateText({
        ...
        modelId: MODEL,
        promptVersion: "task06c-r2e-provider-preflight.v1",
  ...
  const record = {
    schemaVersion: "task06c-r2e-provider-preflight.v1",
    generatedAt,
    modelId: MODEL,
    promptVersion: "task06c-r2e-provider-preflight.v1",
  ...
  const directory = resolve("evaluation/task06c-r2e/preflight");
  ...
  process.stdout.write(
    `Task 06C-R2E provider preflight ${stable ? "PASSED" : "FAILED"}: ${String(attempts.filter((attempt) => attempt.succeeded).length)}/3; record=${filename}\n`,
  );
