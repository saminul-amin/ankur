...
const iterationValue = process.env["ANKUR_TASK06C_R2_ITERATION"] ?? "1";
if (!/^[1-2]$/u.test(iterationValue)) throw new Error("TASK06C_R2_ITERATION_INVALID");
const ITERATION_ROOT = resolve(`evaluation/task06c-r2/iteration-${iterationValue}`);
...
async function observeOperation<T>(input: {
  ...
  if (controlled !== undefined) {
    if (process.env["ANKUR_TASK06C_R2_PRESERVE_FAILURES"] === "true") {
...
async function runLive(state: PrivateState): Promise<void> {
  if (process.env["ANKUR_TASK06C_R2_LIVE_OPT_IN"] !== "true") throw new Error("TASK06C_R2_LIVE_OPT_IN_REQUIRED");
...
async function validateDryRun(): Promise<void> {
  ...
  process.stdout.write(`Task 06C-R2 evaluation dry-run PASSED: 9 materials (6 frozen + 3 holdout), 3 domains, 3 languages, 4 input types, ${String(plannedQuestions)} planned Ankur questions and 45 fixed logical operations.\n`);
}
