// Task 06C-R2G runs the unchanged Task 06C-R2F evaluation design against the
// multilingual generation-reliability corrections. The corpus, denominators,
// answer cases, thresholds, metrics, and export shape are identical to R2F so
// the two runs are directly comparable; only the output root differs, leaving
// every frozen R2F record untouched.
...
const iterationValue = process.env["ANKUR_TASK06C_R2G_RUN"] ?? "1";
if (iterationValue !== "1") throw new Error("TASK06C_R2G_RUN_INVALID");
const ITERATION_ROOT = resolve(`evaluation/task06c-r2g/run-${iterationValue}`);
...
interface PrivateState {
  ...
  semanticDiagnostics: Array<ProviderValidationDiagnostic & { readonly operationId: string }>;
}
...
async function observeOperation<T>(input: {
  ...
  if (controlled !== undefined) {
    if (process.env["ANKUR_TASK06C_R2G_PRESERVE_FAILURES"] === "true") {
  ...
  input.state.semanticDiagnostics = [
    ...input.state.semanticDiagnostics.filter((item) => item.operationId !== input.operationId),
    ...diagnostics.map((diagnostic) => ({ ...diagnostic, operationId: input.operationId })),
  ];
...
async function runLive(state: PrivateState): Promise<void> {
  if (process.env["ANKUR_TASK06C_R2G_LIVE_OPT_IN"] !== "true") throw new Error("TASK06C_R2G_LIVE_OPT_IN_REQUIRED");
  ...
  const learning = new GemmaLearningContentAdapter(
    provider,
...
function fixedReliabilityOperations(state: PrivateState): ProviderOperation[] {
  ...
  if (specifications.length !== 45) throw new Error("R2_LOGICAL_OPERATION_PLAN_INVALID");
...
async function exportPublic(state: PrivateState): Promise<void> {
  ...
  const files: Array<[string, unknown]> = [
    ...
    ["semantic-diagnostics.json", state.semanticDiagnostics.toSorted((left, right) =>
...
async function validateDryRun(): Promise<void> {
  ...
  process.stdout.write(`Task 06C-R2G evaluation dry-run PASSED: 9 materials (6 frozen + 3 holdout), 3 domains, 3 languages, 4 input types, ${String(plannedQuestions)} planned Ankur questions and 45 fixed logical operations.\n`);
}
