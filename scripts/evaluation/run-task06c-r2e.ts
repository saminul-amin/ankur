...
const iterationValue = process.env["ANKUR_TASK06C_R2E_RUN"] ?? "1";
if (!/^[1-2]$/u.test(iterationValue)) throw new Error("TASK06C_R2E_RUN_INVALID");
const ITERATION_ROOT = resolve(`evaluation/task06c-r2e/run-${iterationValue}`);
...
async function observeOperation<T>(input: {
  ...
  if (controlled !== undefined) {
    if (process.env["ANKUR_TASK06C_R2E_PRESERVE_FAILURES"] === "true") {
...
async function runLive(state: PrivateState): Promise<void> {
  if (process.env["ANKUR_TASK06C_R2E_LIVE_OPT_IN"] !== "true") throw new Error("TASK06C_R2E_LIVE_OPT_IN_REQUIRED");
...
async function validateDryRun(): Promise<void> {
  ...
  process.stdout.write(`Task 06C-R2E evaluation dry-run PASSED: 9 materials (6 frozen + 3 holdout), 3 domains, 3 languages, 4 input types, ${String(plannedQuestions)} planned Ankur questions and 45 fixed logical operations.\n`);
}
